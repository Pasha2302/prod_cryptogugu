...
ALLOWED_HOSTS = [
    'itineris.serveo.net', 'wow-coins-server.serveo.net',
    '0.0.0.0', '127.0.0.1', 'localhost',
    '[::1]',  # Для поддержки IPv6
]
...
INSTALLED_APPS = [
    ...
    'channels',  # Добавляем Channels, >> daphne -b 0.0.0.0 -p 8005 src.asgi:application
...
WSGI_APPLICATION = 'src.wsgi.application'  # Для Gunicorn (HTTP)
ASGI_APPLICATION = 'src.asgi.application'  # Для Daphne (WebSocket)

CHANNEL_LAYERS = {
    'default': {
        'BACKEND': 'channels.layers.InMemoryChannelLayer',  # Для разработки
        # Для продакшн используй Redis:
